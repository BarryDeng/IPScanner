...
%.o: %.c
	$(CC) -I$(INCLUDE_DIR) -c $< -o $(OBJ_DIR)/$@

include $(OBJ_DIR)/$(dependence) #注意该句要放在终极目标all的规则之后，否则.d文件里的规则会被误当作终极规则了
%.d: %.c
	set -e; rm -f $(OBJ_DIR)/$@; \
	$(CC) -MM -I$(INCLUDE_DIR) $(CPPFLAGS) $< > $(OBJ_DIR)/$@.$$$$; \
	sed 's,\($*\)\.o[ :]*,\1.o $@ : ,g' < $(OBJ_DIR)/$@.$$$$ > $(OBJ_DIR)/$@; \
	rm -f $(OBJ_DIR)/$@.$$$$

.PHONY: clean #之所以把clean定义成伪目标，是因为这个目标并不对应实际的文件
clean:
	rm -f $(PROG) $(OBJ_DIR)/$(objects) $(OBJ_DIR)/$(dependence)
